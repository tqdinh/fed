...
class FederatedLearning:
    ...
    def setupParams(self):
        ...
        self.St=np.random.randint(1,int(self.m))
...
if __name__ == "__main__":
    ...
    manager.start()
    my_server = manager.FederatedLearning(C,K)
    _today=datetime.now()
    for j in range(NUMBER_OF_ROUND):
        ...
        www=my_server.get_w()
        round_find="{0}/round_{1}".format(folder_today,j)
        write_list(folder_today,www)